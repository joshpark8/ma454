...
\section{Extending field \homo s and the Galois group of an extension}
  ...
  \begin{definition}[$F$-\homo]
    ...
    Then, when \(\sigma : M \to L\) is a \homo , we say that \(\sigma\) is a \tun{\(K\)-\homo} if \(\sigma\) leaves \(\varphi(K)\) pointwise fixed, which is to say that for all \(\alpha \in \varphi(K)\), one has $\sigma(\alpha) = \alpha$.
  \end{definition}
  ...
  \begin{definition}[Galois group of]
    Suppose that $ L:K $ is a field extension.
    With $ \Aut(L) $ denoting the automorphism group of $ L $, we set \begin{align*}
      \Gal(L:K) = \lt\{ \sigma\in \Aut(L) : \sigma \text{ is a $ K $-\homo} \rt\}
    \end{align*}
    and we call $ \Gal(L:K) $ the \tun{Galois group} of $ L:K $.
  \end{definition}

  \tbo{Note:}\quad Proposition 3.1 tells us that when $ f\in K[t] $ and $ \sigma\in \Gal(L:K) $, the mapping $ \sigma $ permutes the roots of $ f $ that lie in $ L $.
  ...
  \begin{definition}[Splitting field, \sfe]
    Suppose that $ L:K $ is a field extension relative to the embedding $ \vphi : K\to L$, and $ f\in K[t]\setminus K $. \begin{enumerate}[label=(\roman*)]
      ...
      \item Suppose that $ f $ splits over $ L $, and let $ M $ be a field with $ \vphi(K)\sseq M\sseq L $.
        We say that \tun{$ M:K $ is a splitting} \tun{field extension for $ f $} if $ M $ is the smallest subfield of $ L $ containing $ \vphi(K) $ over which $ f $ splits.
      \item More generally, suppose that $ S\sseq K[t]\setminus K $ has the property that every $ f\in S $ splits over $ L $.
        ...
        We say that \tun{M:K is a \sfe~for $ S $} if $ M $ is the smallest subfield of $ L $ containing $ \vphi(K) $ over which every polynomial $ f\in S $ splits.
    \end{enumerate}
  \end{definition}
...
% \subsection{The Frobenius map}
  ...
  \tbo{Note: } $ \Fix_\phi(K)=\lt\{ \alpha\in K:\phi(\alpha)=\alpha \rt\} $.
...
% \subsection{Finding roots of quadratic, cubic, and quartic polynomials}
  \begin{definition}[Radical element/extension]
    ...
    We say that $ L:K $ is \tun{an extension by radicals} when there is a tower of field extensions $ L=L_r:L_{r-1}:\cdots:L_0=K $ such that $ L_i=L_{i-1}(\beta_i) $ with $ \beta_i $ radical over $ L_{i-1} $ ($ 1\leq i\leq r $).
